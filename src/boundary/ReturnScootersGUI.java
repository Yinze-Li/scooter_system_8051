...
public class ReturnScootersGUI {
	...
	private void initialize() {
		...
		int slotID = dockList.returnWhich(dockID)+1;		
			...
			public void actionPerformed(ActionEvent event)
			{
				while(!timeout && !Pressed) {
					if(event.getSource() == button[slotID]) {
						...
						userList.unuse(studID);
						dockList.dockSave();
						if (checktime[0]<=30 && checktime[1]<=120) {
							transactionList.newTrans(studID, dockID, slotID-1, false, false);
						}
						else {
							transactionList.newTrans(studID, dockID, slotID-1, false, true);
							userList.ban(studID);
						}
						userList.save();
