...
public class ReturnFailed {
	...
	public ReturnFailed(int studID,int dockID,UserList userList,DockList dockList) {
		this.studID = studID;
		this.dockID = dockID;
	...
	private void initialize() {
		...
		JLabel lblTimeoutFailTo = new JLabel("TIMEOUT! FAILED TO RETURN");
		...
		class BTListener implements ActionListener{
			...
			public void actionPerformed(ActionEvent event)
			{
				if(event.getSource() == btnAbort) {
					...
					ReturnScootersGUI GUI = new ReturnScootersGUI(studID, dockID, userList, dockList);
					frame.dispose();
				}
			}
		}
